...
def hubspot_raw() -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "lead_create_date":   pd.to_datetime(["2025-12-01", "2025-12-15", "2025-12-20"]),
...
class TestMetaConnectorContract:
    ...
    def test_validator_flags_zero_spend_days(self):
        zero = pd.DataFrame({
            ...
            "campaign_id":   ["c1"] * 5,
            "date":          pd.date_range("2026-01-01", periods=5).strftime("%Y-%m-%d").tolist(),
            "spend":         [0.0] * 5,
            "impressions":   [0] * 5,
            "clicks":        [0] * 5,
        })
    ...
    def test_validator_warns_on_spend_drop(self):
        drop = pd.DataFrame({
            ...
            "campaign_id":   ["c1"] * 6,
            "date":          pd.date_range("2026-01-01", periods=6).strftime("%Y-%m-%d").tolist(),
