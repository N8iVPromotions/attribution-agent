...
class MetaConnector:
    ...
    def pull_campaign_insights(self, ad_account_id, start_date, end_date, level="campaign"):
        logger.info(f"[Meta] Pulling insights for {ad_account_id} | {start_date} to {end_date}")
        ...
        for row in rows:
            ...
            for action in row.get("actions", []):
                ...
                if atype in CONVERSION_ACTIONS:
                    base[f"conversions_{atype.replace('.', '_')}"] = int(action.get("value", 0))
...
def pull_meta_data(ad_account_id: str, lookback_days: int = 30, access_token: str = "") -> pd.DataFrame:
